...
export default function ProductPage() {
  ...
  const handleOrder = () => {
    const selectedDetail = product.detail?.[selectedDetailIndex];
    const price = product.flashSale ? selectedDetail.flashSalePrice : selectedDetail.currentPrice;
    const productData = [
      {
        id: product._id,
        name: product.name,
            ...
            {activeTab === 'specifications' && (
              <div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-gray-700">
                  <p>
                    <strong>Brand / Origin:</strong> {product.brand?.name || 'N/A'} / {product.origin || "N/A"}
                  </p>
                  <p>
